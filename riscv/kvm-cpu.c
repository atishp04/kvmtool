...
#define RISCV_CONFIG_REG(id)	__kvm_reg_id(KVM_REG_RISCV_CONFIG, id)
...
struct kvm_cpu *kvm_cpu__arch_init(struct kvm *kvm, unsigned long cpu_id)
{
	...
	reg.id = RISCV_CONFIG_REG(KVM_REG_RISCV_CONFIG_ISA);
	reg.addr = (unsigned long)&isa;
	if (ioctl(vcpu->vcpu_fd, KVM_GET_ONE_REG, &reg) < 0)
		die("KVM_GET_ONE_REG failed (config.isa)");

	reg.id = RISCV_CONFIG_REG(KVM_REG_RISCV_CONFIG_TIMEBASE);
